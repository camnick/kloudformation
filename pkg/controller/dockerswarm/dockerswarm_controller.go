...
import (
	...
	//swarm "github.com/gotopple/kloudformation/pkg/apis/swarm/v1alpha1"
...
func (r *ReconcileSubnet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if !ok {
		...
		r.events.Eventf(instance, `Normal`, `Created`, "Created AWS Subnet (%s)", dockerswarmid)
		...
		if err != nil {
			...
			if ierr != nil {
				...
				r.events.AnnotatedEventf(instance,
					map[string]string{`cleanupSubnetId`: dockerswarmid},
					`Warning`,
					`DeleteFailure`,
		...
		if len(instance.Spec.Tags) >= 1 {
			...
			tagOutput, err := ec2svc.CreateTags(&ec2.CreateTagsInput{
				Resources: []*string{aws.String(dockerswarmid)},
				Tags:      ts,
			})
			if err != nil {
